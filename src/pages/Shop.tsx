...
const Shop = () => {
  ...
  const {
    ...
    enabled: Boolean(!backendCategories.length || active === "all" || activeCategory?.backendId),
    staleTime: 2 * 60 * 1000,
    retry: 1,
  });

  const liveProducts = productsResponse?.data?.map(fromBackendProduct) || [];
  const usingLiveProducts = Boolean(productsResponse?.data);
  ...
  const visibleProducts = usingLiveProducts ? liveProducts : staticProducts;
  const isLoading = (productsLoading || categoriesLoading) && !usingLiveProducts;

...
import { useBackendCategories, useFeaturedProducts } from "@/hooks/useBackendContent";
import { fromBackendCategory, fromBackendProduct, fromStaticProduct } from "@/lib/catalog";
...
const staticFeatured = products.filter((product) => product.badge).slice(0, 4).map(fromStaticProduct);
const staticDessertCats = categories.filter((category) => category.id !== "savory");
...
const Home = () => {
  const { data: backendFeatured = [] } = useFeaturedProducts();
  const { data: backendCategories = [] } = useBackendCategories();

  const featured = backendFeatured.length
    ? backendFeatured.slice(0, 4).map(fromBackendProduct)
    : staticFeatured;

  const fallbackBySlug = new Map<string, (typeof categories)[number]>(categories.map((category) => [category.id, category]));
  const dessertCats = backendCategories.length
    ? backendCategories
        .filter((category) => category.slug !== "savory")
        .slice(0, 5)
        .map((category) => {
          const mapped = fromBackendCategory(category);
          const fallback = fallbackBySlug.get(mapped.slug);
          return {
            id: mapped.slug,
            name: mapped.name,
            image: mapped.image || fallback?.image || staticDessertCats[0].image,
            blurb: mapped.blurb || fallback?.blurb || "Freshly baked selection",
          };
        })
    : staticDessertCats;
    ...
    <Layout>
      ...
      <section className="border-y border-border bg-secondary/60">
        <div className="container flex flex-col items-center justify-center gap-3 py-4 text-center text-sm text-foreground/80 sm:flex-row sm:gap-8">
          ...
          <span className="inline-flex items-center gap-2">
            <MapPin className="h-4 w-4 text-primary" /> Delivering across Chandigarh Tricity
          </span>
        </div>
      </section>
      ...
      <section className="border-y border-border bg-gradient-warm py-16 md:py-24">
        <div className="container grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <img
            src={featured[0]?.image || staticFeatured[0]?.image}
      ...
      <section className="border-y border-border bg-gradient-warm py-14 md:py-16">
        <div className="container grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <h2 className="font-display mb-3 text-3xl md:text-4xl">Across Chandigarh Tricity</h2>
            <p className="text-muted-foreground">
              We deliver fresh-baked goodness across Chandigarh, Mohali and Panchkula. Outside the tricity? Drop us a WhatsApp.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {["Chandigarh", "Mohali", "Panchkula", "Zirakpur", "Kharar", "New Chandigarh"].map((city) => (
